"""
Phase 6: Bidirectional LSTM Model

This script implements a Bidirectional LSTM (Bi-LSTM) for RUL prediction.
Unlike regular LSTM that processes sequences in one direction, Bi-LSTM processes
data both forward and backward, capturing context from both past and future timesteps.

Model Architecture:
    - Input: 50-cycle sequences (50 timesteps x 17 features)
    - Bidirectional LSTM Layer 1: 64 units (forward + backward = 128 total)
    - Bidirectional LSTM Layer 2: 32 units (forward + backward = 64 total)
    - Dense Layer: 32 units with ReLU activation
    - Output: Single value (predicted RUL)
    - Dropout: 0.2 after each layer

Why Bidirectional?
    - Reads sequences forward (past to future) and backward (future to past)
    - Captures richer context by considering both directions
    - Often more accurate than regular LSTM
    - Doubles the parameters but improves prediction quality
